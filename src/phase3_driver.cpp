...
struct RunConfig {
    ...
    double k = 1.0;
    long long max_mcs = 100000;
    unsigned int threads = 0;
    bool dry_run = true;
    bool do_compile = true;
    bool do_lattice = true;
    bool do_smallworld = true;
    bool do_scalefree = true;
};
...
static void run_smallworld(const RunConfig& cfg) {
    ...
    const std::vector<double> betas{0.00,0.001,0.003,0.01,0.03,0.10,0.30,1.00};
    const int K = 4;
    const int graph_realizations = 200;
    const int reps = 100;
    const std::string out_csv = cfg.data_dir + "\\phase3_smallworld_K4.csv";
    const unsigned long long seed0 = 500000ULL;

    std::cout << "\n=== Phase 3 small-world scan ===\n";
    for (int N : Ns) {
...
static void print_usage(const char* prog) {
    std::cout
        ...
        << "  --max-mcs M          max_mcs cap (default: 100000)\n"
        << "  --help               Show this help\n\n"
        << "Examples:\n"
        << "  phase3_driver.exe --run\n"
        << "  phase3_driver.exe --run --smallworld-only --threads 8\n"
        << "  phase3_driver.exe --root C:\\Users\\USER\\Desktop\\C++ project\\rps-criticality --run\n";
}
